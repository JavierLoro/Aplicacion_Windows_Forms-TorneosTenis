Public Class TorneoDAO
    ...
    Public Sub LeerTodas(ruta As String)
        ...
        col = AgenteBD.ObtenerAgente(ruta).Leer("SELECT * FROM Torneos ORDER BY idTorneo")
        For Each aux In col
    ...
    Public Function comprobarEdicion(ByVal t As Torneos, anualidad As Integer) As Integer
        Return AgenteBD.ObtenerAgente.Modificar("SELECT Ediciones.Anualidad FROM Torneos INNER JOIN Ediciones ON Torneos.idTorneo = Ediciones.Torneo WHERE Ediciones.Anualidad=" & anualidad & "AND Torneos.idTorneo=" & t.idTorneos & ";")
    End Function
    ...
    Friend Function devolvercuadro(torneos As Torneos, anualidad As Integer) As List(Of String)
        Dim col As Collection : Dim aux As Collection
        Dim lista As New List(Of String)
        col = AgenteBD.ObtenerAgente.Leer("SELECT Juegos.Jugadora, Juegos.Set_1, Juegos.Set_2, Juegos.Set_3 FROM Torneos INNER JOIN (Ediciones INNER JOIN (Partidos INNER JOIN Juegos ON Partidos.idPartido = Juegos.Partido) ON Ediciones.Anualidad = Partidos.Anualidad) ON Torneos.idTorneo = Ediciones.Torneo WHERE Ediciones.Anualidad=" & anualidad & " AND Ediciones.Torneo=" & torneos.idTorneos & " ORDER BY  Juegos.Partido;")
        For Each aux In col
